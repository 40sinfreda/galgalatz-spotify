...
Pulls new songs from PlayingList.net's Galgalatz playlist and adds them to your playlist,
planting your chosen artists every 10 songs.
"""
...
def get_playlist_tracks(token, playlist_id, limit=50):
    """Get the most recent tracks from a playlist."""
    resp = requests.get(
        f"{SPOTIFY_API_BASE}/playlists/{playlist_id}/tracks",
        headers={"Authorization": f"Bearer {token}"},
        params={"limit": limit, "fields": "items(track(uri,name,artists(name)))", "market": "IL"},
        timeout=10,
    )
...
def main():
    ...
    # Get current songs from my playlist
    my_uris = set(get_my_playlist_uris(token, playlist_id))
    known_uris = seen_uris | my_uris

    # Get latest songs from Galgalatz playlist (PlayingList.net)
    galgalatz_tracks = get_playlist_tracks(token, GALGALATZ_PLAYLIST_ID, limit=50)
    print(f"Fetched {len(galgalatz_tracks)} tracks from Galgalatz playlist")

    # Find new tracks not yet in our playlist
    new_tracks = [t for t in galgalatz_tracks if t["uri"] not in known_uris]
    print(f"New tracks to add: {len(new_tracks)}")

    if not new_tracks:
        print("No new tracks found, skipping")
        save_state(state)
        return
